...
text: '```🚩 Akses Bot Ke Private Chat Di Tolak, Upgrade Premium Hanya 5.000 Idr Agar Bisa Bebas Akses Bot Dengan Hubungi Owner :\nwa.me/263734408604\n\n• Join Ke Group Official Bot Untuk Free Akses & Informasi Kedepannya Tentang Bot : https://chat.whatsapp.com/K3NYSm9bju2GuorpWFxKvO```',
contextInfo: {
...
body: '2023 - 2025 © Kemii Salsabila',
...
global.dfail = (type, m, conn) => {
    ...
    let msg = {
        rowner: '*ID: 🇮🇩*\n```Maaf, command ini hanya dapat digunakan oleh Real Owner```\n\n*EN: 🇺🇲*\n```Sorry, this command can only be used by the real owner```',
		owner: '*ID: 🇮🇩*\n```Maaf, command ini hanya dapat digunakan oleh owner\n\n*EN: 🇺🇲*\n```Sorry, this command can only be used by the owner```',
		mods: '*ID: 🇮🇩*\n```Maaf, command ini hanya dapat digunakan oleh moderator```\n\n*EN: 🇺🇲*\n```Sorry, this command can only be used by moderator```',
		premium: '*ID: 🇮🇩*\n```Maaf, command ini hanya untuk user premium.```\n\n*EN: 🇺🇲*\n```Sorry, this command is for premium users only.```',
		group: '*ID: 🇮🇩*\n```Maaf, command ini hanya dapat digunakan di group.```\n\n*EN: 🇺🇲*\n```Sorry, this command can only be used in groups.```',
		vip: '*ID: 🇮🇩*\n```Maaf, command ini hanya untuk user vip.```\n\n*EN: 🇺🇲*\n```Sorry, this command is for vip users only.```',
		private: '*ID: 🇮🇩*\n```Maaf, command ini hanya dapat digunakan di Pesan Pribadi.```\n\n*EN: 🇺🇲*\n```Sorry, this command can only be used in Private Chat.```',
		admin: '*ID: 🇮🇩*\n```Maaf, command ini hanya untuk admin grup.```\n\n*EN: 🇺🇲*\n```Sorry, this command is only for group admins.```',
		botAdmin: '*ID: 🇮🇩*\n```Jadikan bot sebagai Admin untuk menggunakan command ini.```\n\n*EN: 🇺🇲*\n```Make the bot an Admin to use this command.```',
